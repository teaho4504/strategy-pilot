...
from app.services.kiwoom_session import get_active_kiwoom_session
...
class KiwoomClient:
    async def request_tr(
        ...
        if session is None and not settings.credentials_configured:
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                response = await client.post(f"{base_url}{spec.service_uri}", json=request_payload, headers=headers)
