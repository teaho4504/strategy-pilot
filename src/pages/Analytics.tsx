...
import { useAnalyticsQuery } from "@/services/adapters";
...
export default function Analytics() {
  ...
  const { data: a, isLoading, isError } = useAnalyticsQuery(range);
  ...
  return (
    <>
      ...
      <main className="space-y-3 p-4 animate-fade-in">
        ...
        {isLoading && <Card className="text-center text-sm text-muted-foreground">분석 데이터를 불러오는 중입니다.</Card>}
        {isError && <Card className="text-center text-sm text-danger">분석 데이터를 불러오지 못했습니다.</Card>}
        {a && (
          <>
            <Card>
